#!/usr/bin/env python3
"""Generate five self-contained hover-state options into prototypes/why/.

Each file is standalone. They share a shell deliberately: the only thing that
differs is the one line added to the hover card, which is the whole point.
"""
import os, json

OUT = "/Users/treyfour/dev/consensus/.claude/worktrees/relevance/prototypes/why"
...
.hd p{color:#a1a1aa;font-size:var(--t-xs);line-height:1.55;margin-top:2px;max-width:64rem}
.hd .cost{display:inline-block;font-family:var(--mono);font-size:var(--t-2xs);background:#ffffff1f;
  border-radius:625rem;padding:1px var(--s-xs);margin-top:var(--s-xs)}
.hd .cost.zero{background:var(--pos);color:#fff}
.hd .cost.some{background:var(--warn);color:#fff}
.wrap{display:grid;grid-template-columns:1fr 24rem;height:calc(100vh - 78px);overflow:hidden}
.wrap>*{min-height:0}

/* the thread, kept short — it is context, not the subject */
.thread{overflow-y:auto;padding:var(--s-xl) var(--s-2xl);border-right:1px solid var(--bd)}
.inner{max-width:40rem;margin:0 auto}
...
.s{font-size:var(--t-sm);line-height:1.7;margin-bottom:var(--s-sm);max-width:36rem;
  padding-left:var(--s-xs);border-left:2px solid transparent;transition:background .12s,border-color .12s}
...
/* references: the list as it is today */
...
.row:hover{border-color:var(--accent-bd);background:var(--accent-subtle)}
...
/* the existing hover card. Everything above the rule is what it shows today. */
.card{position:fixed;z-index:60;width:23rem;background:#fff;border:1px solid var(--bd);
...
.shim{height:.62rem;border-radius:625rem;background:linear-gradient(90deg,var(--faint),var(--subtle),var(--faint));
  background-size:200% 100%;animation:sh 1.1s linear infinite;margin-bottom:4px}
.shim.b{width:72%}
...
<body>
<div class="hd"><b>__NAME__</b><p>__DESC__</p><span class="cost __COSTCLASS__">__COST__</span></div>
...
<div class="wrap">
  ...
  <aside class="pane">
    <div class="pane-hd"><b>References</b><span>20 included · 6 cited</span></div>
    <div class="list" id="list"></div>
  </aside>
</div>
<div class="card" id="card"></div>
<div class="note">Real data: six papers, three sub-queries and the funnel captured from the live CRISPR
thread. Passage counts are the product’s own for Wienert (27); the other five are illustrative.</div>

<script>
const SQ=["CRISPR off-target effects in vivo therapies",
          "CRISPR off-target editing, genome-wide off-target profiling",
          "CRISPR nuclease specificity, delivery, immunogenicity"];
const P=__P__;
const A=__A__;
...
$('ans').innerHTML=A.map((a,k)=>'<p class="s" data-s="'+k+'">'+esc(a.s)+' '+
  a.srcs.map(i=>'<span class="cite">'+P[i].n.toUpperCase()+'</span>').join(' ')+'</p>').join('');

$('list').innerHTML=P.map((p,i)=>'<div class="row" data-p="'+i+'"><h3>'+esc(p.t)+'</h3>'+
  '<div class="m">'+esc(p.a)+' · '+p.y+' · '+p.c+' cites · '+esc(p.j)+'</div></div>').join('');
...
function place(row){
  ...
  card.style.top=Math.max(84,Math.min(r.top,window.innerHeight-h-28))+'px';
}
function lit(i){
  document.querySelectorAll('.s').forEach(el=>el.classList.toggle('lit',
    i!=null&&A[+el.dataset.s].srcs.includes(i)));
}
__WHYJS__

document.querySelectorAll('.row').forEach(row=>{
  const i=+row.dataset.p;
  row.addEventListener('mouseenter',()=>{card.classList.add('on');show(i,row)});
  row.addEventListener('mouseleave',()=>{card.classList.remove('on');lit(null)});
});
</script>
</body>
</html>
...
P = [
 dict(n="Wienert 2022", t="CRISPR nuclease off-target activity and mitigation strategies",
      a="B. Wienert", y=2022, c=46, j="Frontiers in Genome Editing", type="LITERATURE REVIEW",
      sq=2, q=27,
      ...
      why2="Framed the open questions your answer opens with.",
      why3="A review of off-target activity and mitigation. You asked what is unresolved, and this is where the answer's “unmet need” framing comes from."),
 dict(n="Höijer 2021", t="CRISPR-Cas9 induces large structural variants at on- and off-target sites",
      a="I. Höijer", y=2021, c=149, j="Nature Communications", type="PRIMARY RESEARCH",
      sq=1, q=19,
      ...
      why2="The evidence that large rearrangements go undetected.",
      why3="You asked about in vivo therapies. This is the primary evidence that short-read assays miss the large events, using long reads."),
 dict(n="Cancellieri 2022", t="Human genetic diversity alters off-target outcomes of therapeutic gene editing",
      a="S. Cancellieri", y=2022, c=98, j="Nature Genetics", type="PRIMARY RESEARCH",
      sq=1, q=14,
      ...
      why2="The only paper here on population variation.",
      why3="Directly on the “biological variability” your question names: it shows a reference-genome analysis under-reports risk for individuals."),
 dict(n="Kalter 2025", t="Off-target effects in CRISPR-Cas genome editing for human therapeutics",
      a="N. Kalter", y=2025, c=56, j="Molecular Therapy Nucleic Acids", type="LITERATURE REVIEW",
      sq=1, q=23,
      ...
      why2="Surveyed the detection standards your question asks about.",
      why3="Closest match to your question. It surveys detection methods for therapeutic editing and argues the standards disagree."),
 dict(n="Angelini Stewart 2025", t="Measurement and clinical interpretation of CRISPR off-targets",
      a="A. Angelini Stewart", y=2025, c=6, j="Nature Genetics", type="LITERATURE REVIEW",
      sq=1, q=11,
      ...
      why2="Covers post-delivery monitoring, which nothing else here does.",
      why3="Answers the monitoring half of your question: what to measure clinically once an edit has been delivered."),
 dict(n="Huang 2022", t="In vivo delivery of CRISPR-Cas9 genome editing components",
      a="K. Huang", y=2022, c=39, j="Biomaterials", type="LITERATURE REVIEW",
      sq=2, q=8,
      ...
      why2="Delivery, which your question names but the other five barely touch.",
      why3="Only on delivery. Relevant to the “in vivo” half of your question, and not to off-target effects at all."),
]
...
OPTIONS = [
 dict(slug="assembled", name="1 · Assembled",
   desc="The line is built from things the pipeline already emitted: which of the three sub-queries retrieved the paper, how many passages matched, and whether the answer ended up citing it. No model writes it. Hover any reference.",
   cost="zero extra compute · string templating", costclass="zero",
   js=r"""
function show(i,row){
  const p=P[i], used=A.some(a=>a.srcs.includes(i));
  card.innerHTML=base(i)+
    '<div class="why"><span class="k">Why this paper</span><span class="v">'+
 ...
 dict(slug="written", name="2 · Written with the answer",
   desc="When the model writes the answer it also emits one short clause per source, in the same completion. Twenty one-liners for a couple of hundred extra output tokens on a call that was already happening.",
   cost="~200 output tokens, on an existing call", costclass="some",
   js=r"""
function show(i,row){
  card.innerHTML=base(i)+
    '<div class="why"><span class="k">Why this paper</span><span class="v">'+esc(P[i].why2)+'</span></div>';
  place(row); lit(i);
}"""),
 dict(slug="on-demand", name="3 · Written on hover",
   desc="Nothing is precomputed. Hovering asks for one sentence about this paper against this question, and the result is cached, so cost follows curiosity rather than result-set size. Hover a second time to see the cache.",
   cost="one small call per hovered paper", costclass="some",
   js=r"""
const cache={};
function show(i,row){
  if(cache[i]){
    card.innerHTML=base(i)+'<div class="why"><span class="k">Why this paper</span><span class="v">'+
 ...
 dict(slug="own-words", name="4 · In the paper’s own words",
   desc="Nothing is generated at all. Retrieval already scored every passage in the paper against the query, so the top-scoring one is shown verbatim. The paper explains itself, and there is nothing to hallucinate.",
   cost="zero extra compute · already ranked", costclass="zero",
   js=r"""
function show(i,row){
  card.innerHTML=base(i)+
    '<div class="why"><span class="k">Why this paper</span><span class="v"><q>'+esc(P[i].passage)+'</q>'+
 ...
 dict(slug="points", name="5 · Points at the answer",
   desc="No new sentence anywhere. Hovering a reference highlights the sentence in the answer it holds up, and the card says how many. The explanation is the answer you already read.",
   cost="zero extra compute · no new words", costclass="zero",
   js=r"""
function show(i,row){
...
for o in OPTIONS:
    html = (TEMPLATE
        ...
        .replace("__DESC__", o["desc"])
        .replace("__COST__", o["cost"])
        .replace("__COSTCLASS__", o["costclass"])
        .replace("__P__", json.dumps(P, ensure_ascii=False))
        .replace("__A__", json.dumps(A, ensure_ascii=False))
        .replace("__WHYJS__", o["js"]))
    path = os.path.join(OUT, "option-%s-%s.html" % (OPTIONS.index(o)+1, o["slug"]))
